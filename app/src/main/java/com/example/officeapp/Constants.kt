package com.example.officeapp

object Constants {
    const val ENTERED_NICK="entered_nick"
    const val ENTERED_PASS="entered_pass"
    const val R_USER_NICK="r_user_nick"
    const val R_USERNAME="r_username"
    const val R_USERPASS="r_userpass"
    const val R_USERSTATUS="r_status"

    const val LOGGEDIN_USER="loggedin_user"
    const val LOGGEDIN_USERID="loggedin_userid"
    const val LOGGEDIN_USERNAME="loggedin_username"
    const val LOGGEDIN_USERNICK="loggedin_usernick"
    const val DATE_LIST="date_list"
    const val CHOSEN_DATE="chosen_date"
    const val SEVEN_DAYS="seven_days"
}